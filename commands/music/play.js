...
module.exports = {
    ...
    async execute(message, args, ops) {
        ...
        async function play(song) {
            ...
            if (!song) {
                if (queue.songs.length == 0) {
                    queue.leaveTimer = setTimeout(function () {
                        leave_with_timeout(message.guild.id);
                    }, 300 * 1000);
                }
        ...
        async function leave_with_timeout(guild_id) {
            const serverQueue = ops.queue.get(guild_id);
            if (serverQueue) {
                // serverQueue.textChannel.send('nite nite dooby');
                serverQueue.voiceChannel.leave();
                ops.queue.delete(guild_id);
            }
        }
    }
